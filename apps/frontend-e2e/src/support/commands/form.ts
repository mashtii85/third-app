...
const formSelect = (name: string, option: string, force = false): any =>
...
const formTextarea = (name: string, text: string, force = false): any =>
  cy.get(`textarea[name="${name}"]`).type(text, { force })

const formTextInput = (name: string, text: string, force = false): any =>
  cy.get(`input[name="${name}"]`).clear().type(text, { force })

const hasField = (fieldType: string, name: string, force = false): any => {
  switch (fieldType) {
    case 'select':
      // @ts-ignore: Unreachable code error
      return cy
